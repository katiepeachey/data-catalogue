  ...
  const body = `
    ...
    <script>
      ...
      function toggleOptionsClassifier(checkbox) {
        ...
        var rows = document.querySelectorAll('#fieldConfigTable .field-card');
        if (!checkbox.checked) {
          if (clearHidden) clearHidden.value = '1';
          if (updatedHidden) updatedHidden.value = '';
          if (chipsDiv) chipsDiv.style.opacity = '0.35';
          // Swap chip UI → plain text input for each field card
          ...
          // Swap plain text input → chip UI for each field card
          rows.forEach(function(card, i) {
            var hidden = document.getElementById('exVal_' + i);
            if (!hidden) return;
            ...
            if (!document.getElementById('adminChips_' + i)) {
              ...
              optInput.style.cssText = 'margin-top:5px;width:100%;';
      ...
      function addFieldRow() {
        ...
        headerRight.innerHTML = '<label class="field-vis-label"><input type="checkbox" name="fields[' + i + '][visible]" value="1" checked style="accent-color:#343539;width:14px;height:14px;" /> Visible</label>'
          + '<label class="field-vis-label"><input type="checkbox" name="fields[' + i + '][isSubField]" value="1" style="accent-color:#343539;width:14px;height:14px;" /> Sub-field</label>'
